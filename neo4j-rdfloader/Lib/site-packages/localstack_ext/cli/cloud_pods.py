...
from localstack_ext.bootstrap.pods.remote import RemoteConfig
from localstack_ext.bootstrap.pods.utils.common import is_comma_delimited_list
from localstack_ext.cli.cli_help import delete_help,list_help,load_help,save_help,versions_help
...
@click.option(_G,help='A remote storage URL to save the Cloud Pod to (optional)')
@publish_invocation
def cmd_pod_save(url_or_name=_A,url=_A,remote=_A,services=_A,message=_A,visibility=_A):
	G='public';C=visibility;B=services;A=url_or_name;from localstack_ext.bootstrap import pods_client as D
	if not _is_host_reachable():return
	H=urlparse(A).scheme=='file'
	if url or H:
		E=url or A
		if D.export_pod(target=E):console.print(f"Cloud Pod {E} successfully exported");return _C
		else:console.print(f"[red]Error:[/red] Failed to create Cloud Pod {E}");return _B
	if not licensing.is_logged_in():console.print(_E);return
	if B and not is_comma_delimited_list(B):console.print('[red]Error:[/red] Input the services as a comma-delimited list');return _B
	if C:
		if C not in[G,'private']:console.print('[red]Error:[/red] Possible values for visibility are `public` and `private`');return
		F=D.set_public(pod_name=A,public=C==G)
		if F:console.print(f"Cloud Pod {A} made {C}")
		else:console.print('[red]Error:[/red] Visibility change failed')
		return
	I=[A.strip()for A in B.split(',')]if B else _A;F=D.push_state(pod_name=A,comment=message,services=I,remote_config=RemoteConfig(remote_url=remote))
	if F:console.print(f"Cloud Pod {A} successfully created")
	else:console.print(f"[red]Error:[/red] Failed to create Cloud Pod {A}")
@pod.command(name='load',short_help='Load the state of a Cloud Pod into the application runtime',help=load_help)
@click.argument(_F)
@click.option(_G,help='A remote storage URL to load the Cloud Pod from (optional)')
@publish_invocation
def cmd_pod_load(url_or_name=_A,url=_A,remote=_A):
	B=url;A=url_or_name;from localstack_ext.bootstrap import pods_client as D
	if not _is_host_reachable():return
	def E(_result):
	...
	F=urlparse(A).scheme in['file','http','https','git']
	if B or F:C=D.import_pod(source=B or A);E(C);return C
	if not licensing.is_logged_in():console.print(_E);return
	C=D.pull_state(pod_name=A,remote_config=RemoteConfig(remote_url=remote));E(C)
